...
const LatestMovies = () => {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                const userDocRef = doc(db, 'LatestMovie', id);
    ...
    const colRef = collection(db, "LatestMovie")
    ...
    const saveData = async (downloadURL, id) => {
        const movieRef = doc(db, "LatestMovie", id);
    ...
    const TrailerUpdate = async (id) => {
        if (img) {
            ...
            if (img) {
                const storageRef = ref(storage, "LatestTrailer/" + v4());
                const uploadTask = uploadBytesResumable(storageRef, img);
