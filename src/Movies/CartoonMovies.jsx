...
const CartoonMovies = () => {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                const userDocRef = doc(db, 'Cartoon', id);
                const userDocSnapshot = await getDoc(userDocRef);
                if (userDocSnapshot.exists()) {
                    Cartoon({ id: userDocSnapshot.id, ...userDocSnapshot.data() });
                } else {
    ...
    const colRef = collection(db, "Cartoon")
    ...
    const OneUpdate = async (id) => {
        if (img) {
            ...
            if (img) {
                const storageRef = ref(storage, "CScreen/" + v4());
    ...
    const saveData = async (downloadURL, id) => {
        const movieRef = doc(db, "Cartoon", id);
    ...
    const MobileImgUpdate = async (id) => {
        if (img) {
            ...
            if (img) {
                const storageRef = ref(storage, "Ccreen/" + v4());
                const uploadTask = uploadBytesResumable(storageRef, img);
    ...
    const TrailerUpdate = async (id) => {
        if (img) {
            ...
            if (img) {
                const storageRef = ref(storage, "CTrailer/" + v4());
                const uploadTask = uploadBytesResumable(storageRef, img);
